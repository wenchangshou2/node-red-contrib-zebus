...
<head>
    <title>Code coverage report for src/websocket.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> websocket.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1x</span></td><td class="text"><pre class="prettyprint lang-js">import { Backoff } from "./backoff/backoff";
...
&nbsp;
<span class="fstat-no" title="function not covered" >    get u</span>nderlyingWebsocket(): WebSocket | undefined {
<span class="cstat-no" title="statement not covered" >        return this.websocket;</span>
    }
&nbsp;
    public send(data: string | ArrayBufferLike | Blob | ArrayBufferView): void {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (this.closedByUser)
<span class="cstat-no" title="statement not covered" >            return;</span>
        <span class="missing-if-branch" title="if path not taken" >I</span>if (this.websocket === undefined || this.websocket.readyState !== this.websocket.OPEN)
<span class="cstat-no" title="statement not covered" >            this.buffer?.write([data]);</span>
        else
            this.websocket.send(data);
    }
&nbsp;
<span class="fstat-no" title="function not covered" >    public c</span>lose(code?: number, reason?: string): void {
<span class="cstat-no" title="statement not covered" >        this.closedByUser = true;</span>
<span class="cstat-no" title="statement not covered" >        this.websocket?.close(code, reason);</span>
    }
&nbsp;
...
&nbsp;
<span class="fstat-no" title="function not covered" >    public r</span>emoveEventListener&lt;K extends WebsocketEvents&gt;(
        type: K,
        listener: (instance: Websocket, ev: WebsocketEventMap[K]) =&gt; any,
...
&nbsp;
    private handleErrorEvent = <span class="fstat-no" title="function not covered" >(e</span>v: Event) =&gt; <span class="cstat-no" title="statement not covered" >this.handleEvent(WebsocketEvents.error, ev);</span>
&nbsp;
    ...
    private handleEvent&lt;K extends WebsocketEvents&gt;(type: K, ev: WebsocketEventMap[K]) {
        switch (type) {
            case WebsocketEvents.close:
                <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.closedByUser) // failed to connect or connection lost, try to reconnect
                    this.reconnect();
                break;
            case WebsocketEvents.open:
                this.retries = 0;
                this.backoff<span class="branch-0 cbranch-no" title="branch not covered" >?.r</span>eset(); // reset backoff
                this.buffer<span class="branch-1 cbranch-no" title="branch not covered" >?.forEach(this.send.bind(this));</span>
                this.buffer<span class="branch-1 cbranch-no" title="branch not covered" >?.clear();</span>
                break;
        }
        this.dispatchEvent&lt;K&gt;(type, ev); // 转发所有的消息
    }
&nbsp;
    private reconnect() {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (this.backoff === undefined) // no backoff, we're done
<span class="cstat-no" title="statement not covered" >            return;</span>
